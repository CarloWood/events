...
void my_callback(MyEventType const& CWDEBUG_ONLY(event))
{
...
struct Foo
{
  void callback(MyEventType const& CWDEBUG_ONLY(event)) const
  {
    DoutEntering(dc::notice, "Foo::callback(" << event << ")");
  }

  void callback_with_cookie(MyEventType const& CWDEBUG_ONLY(event), double CWDEBUG_ONLY(cookie)) const
  {
