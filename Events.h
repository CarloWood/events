...
class Request
{
 ...
 public:
  ...
  {
    Dout(dc::notice, "Constructing Request [" << this << "]");
  }
...
class RequestWithBI final : public Request<TYPE>
{
 ...
 public:
  RequestWithBI(std::function<void(TYPE const&)>&& callback, BusyInterface* busy_interface) :
      Request<TYPE>(std::move(callback)), m_busy_interface(busy_interface) { Dout(dc::notice, "Constructing RequestWithBI [" << this << "]"); }
...
{
  DoutEntering(dc::notice, "RequestWithBI<" << libcwd::type_info_of<TYPE>().demangled_name() << ">::handle(" << data << ") [" << (void*)this << "]");
